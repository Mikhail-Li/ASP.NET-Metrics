...
{
    public class CpuApiControllerTests
    {
        private readonly CpuMetricsControllerApi _controllerApi;
        private readonly Mock<ICpuMetricsRepositoryApi> _repositoryApi;
        private readonly Mock<ILogger<CpuMetricsControllerApi>> _logger;
        private readonly Mock<IMetricsAgentClient> _metricsAgentClient;
        private readonly Mock<IAgentsRepository> _agentsRepository;

        public CpuApiControllerTests()
        {
            _repositoryApi = new Mock<ICpuMetricsRepositoryApi>();
            _logger = new Mock<ILogger<CpuMetricsControllerApi>>();
            _metricsAgentClient = new Mock<IMetricsAgentClient>();
            _agentsRepository = new Mock<IAgentsRepository>();

            var config = new MapperConfiguration(cfg => cfg.CreateMap<CpuMetricsApi, CpuMetricApiDto>()
                .ForMember("Time", dto => dto.MapFrom(metric => metric.Transfer(metric.Time))));
            var mapper = config.CreateMapper();

            _controllerApi = new CpuMetricsControllerApi(_logger.Object, _metricsAgentClient.Object, _agentsRepository.Object, _repositoryApi.Object, mapper);
        }
        ...
        public void Create_ShouldCall_Create_FromAgent_ReturnOk()
        {
            ...
            var checkList = fixture.Create<AllCpuMetricApiResponse>();
            var agenturl = fixture.Create<string>();

            _metricsAgentClient.Setup(metrics => metrics.GetAllCpuMetrics(It.IsAny<GetAllCpuMetricsApiRequest>())).Returns(checkList);
           ...
           var agentId = 1;
            ...
            var action = (AllCpuMetricApiResponse)result.Value;

            _metricsAgentClient.Verify(metrics => metrics.GetAllCpuMetrics(It.IsAny<GetAllCpuMetricsApiRequest>()), Times.AtMostOnce());
        ...
        {
            var fixture = new Fixture();
            var checkList = fixture.Create<List<CpuMetricsApi>>();
            ...
            var action = (List<CpuMetricApiDto>)result.Value;
        ...
        {
            var fixture = new Fixture ();
            var check = fixture.Create<CpuMetricsApi>();
            ...
            var action = (CpuMetricApiDto)result.Value;
