...
{
    public class HddApiControllerTests
    {
        private readonly HddMetricsControllerApi _controllerApi;
        private readonly Mock<IHddMetricsRepositoryApi> _repositoryApi;
        private readonly Mock<ILogger<HddMetricsControllerApi>> _logger;
        private readonly Mock<IMetricsAgentClient> _metricsAgentClient;
        private readonly Mock<IAgentsRepository> _agentsRepository;

        public HddApiControllerTests()
        {
            _repositoryApi = new Mock<IHddMetricsRepositoryApi>();
            _logger = new Mock<ILogger<HddMetricsControllerApi>>();
            _metricsAgentClient = new Mock<IMetricsAgentClient>();
            _agentsRepository = new Mock<IAgentsRepository>();

            var config = new MapperConfiguration(cfg => cfg.CreateMap<HddMetricsApi, HddMetricApiDto>()
                .ForMember("Time", dto => dto.MapFrom(metric => metric.Transfer(metric.Time))));
            var mapper = config.CreateMapper();

            _controllerApi = new HddMetricsControllerApi(_logger.Object, _metricsAgentClient.Object, _agentsRepository.Object, _repositoryApi.Object, mapper);
        }
        ...
        public void Create_ShouldCall_Create_FromAgent_ReturnOk()
        {
            ...
            var checkList = fixture.Create<AllHddMetricApiResponse>();
            var agenturl = fixture.Create<string>();

            _metricsAgentClient.Setup(metrics => metrics.GetAllHddMetrics(It.IsAny<GetAllHddMetricsApiRequest>())).Returns(checkList);
            ...
            var action = (AllHddMetricApiResponse)result.Value;

            _metricsAgentClient.Verify(metrics => metrics.GetAllHddMetrics(It.IsAny<GetAllHddMetricsApiRequest>()), Times.AtMostOnce());
        ...
        {
            var fixture = new Fixture();
            var checkList = fixture.Create<List<HddMetricsApi>>();
            ...
            var action = (List<HddMetricApiDto>)result.Value;
        ...
        {
            var fixture = new Fixture();
            var check = fixture.Create<HddMetricsApi>();
            ...
            var action = (HddMetricApiDto)result.Value;
